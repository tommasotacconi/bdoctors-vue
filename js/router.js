...
<<<<<<< HEAD
  return (
    ...
    (store.isAuthenticated = (
      await axios.get(store.apiUri.slice(0, -4) + 'user/status')
    ).data.authenticated)
  );
}

async function checkAuthorization(
  { name: toName, meta: { requiredAuthValue } },
  from,
) {
  if (requiredAuthValue !== undefined) {
    if (requiredAuthValue && !(await getAuthStatus()))
      return toName !== 'dashboard' ? false : { name: 'login' };
    else if (!requiredAuthValue && (await getAuthStatus()))
      return toName === 'login'
        ? { name: 'dashboard' }
        : from.name
          ? false
...
>>>>>>> 55bc5faebd0e6f9881548ef71611d362fd266a54
}
    ...
    children: [
      {
        ...
        children: [
          ...
          { path: '/cerca/:name', name: 'search', component: DoctorShow },
        ],
      },
      {
        path: '/utente',
